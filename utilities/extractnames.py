...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("الاستخدام:")
        print("  1. استخراج اسم مستخدم لفيديو واحد: python your_script_name.py <VIDEO_ID>")
        print("     مثال: python your_script_name.py 7491743835744144662")
        print("  2. استخراج أسماء المستخدمين من ملف: python your_script_name.py <input_file.txt>")
        print("     مثال: python your_script_name.py video_ids.txt")
        sys.exit(1)
    ...
    if os.path.isfile(input_arg):
        ...
        print(f"جاري معالجة معرفات الفيديو من الملف: {input_file_path} باستخدام 10 عمال متزامنين.")
        ...
        # max_workers=10 ليعالج 10 فيديوهات في نفس الوقت
        with ThreadPoolExecutor(max_workers=70) as executor:
            # إرسال المهام إلى الـ executor
            # future_to_id هو قاموس يربط كل Future بمعرف الفيديو الأصلي الخاص به
            future_to_id = {executor.submit(get_tiktok_username, vid_id): vid_id for vid_id in ids_to_process}

            # معالجة النتائج فور اكتمال كل مهمة
            with open(output_file_name, 'w') as outfile:
                for future in as_completed(future_to_id):
                    ...
                    video_id, username = future.result() # استرجاع النتيجة من المهمة المكتملة

                    if username:
                        outfile.write(f"{video_id} : {username}\n")
                        found_count += 1
                    else:
                        if video_id not in deleted_ids: # التأكد من عدم إضافة المعرف المكرر
        ...
        video_id_from_args = input_arg
        print(f"جاري استخراج اسم المستخدم للفيديو ID: {video_id_from_args}")

        if video_id_from_args in deleted_ids:
            print(f"معرف الفيديو {video_id_from_args} موجود بالفعل في قائمة المعرفات المحذوفة. تم التجاوز.", file=sys.stderr)
            sys.exit(0)

        # استدعاء get_tiktok_username مباشرةً
        video_id_result, username_result = get_tiktok_username(video_id_from_args)

        if username_result:
            print(f"اسم المستخدم للفيديو ID {video_id_result}: {username_result}")
        else:
            print(f"لم يتم العثور على اسم المستخدم للفيديو ID: {video_id_result}.", file=sys.stderr)
            print("قد يكون معرف الفيديو غير صالح، أو الفيديو خاص/محذوف، أو هناك مشكلة في الاتصال بالإنترنت.", file=sys.stderr)
            if video_id_result not in deleted_ids:
                save_deleted_id(video_id_result)
                print(f"تمت إضافة {video_id_result} إلى {DELETED_IDS_FILE}.", file=sys.stderr)
            sys.exit(1)