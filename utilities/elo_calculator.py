# حساب النقاط باستخدام نظام Elo
def calculate_expected_score(rating_a, rating_b):
    # تقليل تأثير فرق النقاط بجعل معامل الانحدار أصغر
    return 1 / (1 + 10 ** ((rating_b - rating_a) / 800))  # استخدم 800 بدلاً من 400

def update_ratings_multiple(ranked_videos, data, K=54):
    for i, (vid_i, rating_i, rank_i) in enumerate(ranked_videos):
        for j, (vid_j, rating_j, rank_j) in enumerate(ranked_videos[i+1:], i+1):
            if i == j:
                continue
            ...
            # تعديل معامل K بناءً على الاقتراح
            if score_i > score_j:  # Player i wins
                if data[vid_i]['win_streak'] >= 3:  # Apply increased K only if i is on a win streak
                ...
                if data[vid_i]['loss_streak'] >= 3:  # Apply increased K only if i is on a loss streak
                    K_i = 100 + (data[vid_i]['loss_streak'] - 3) * 30
                else:
                    K_i = K  # Use normal K if not on a loss streak
                if data[vid_j]['win_streak'] >= 3:  # Apply increased K only if j is on a win streak
