        ...
        try:
            ...
            for attempt in range(3):
                try:
                    ...
                    if attempt == 2:
                        raise e
                    print(f"    ⚠ إعادة المحاولة...")
                    time.sleep(3)
            
            print(f"[2] انتظار تحميل المحتوى...")
            time.sleep(5)
            
            print(f"[3] البحث عن عناصر الفيديو...")
            wait_for_videos_smart(page)
            
            print(f"[4] جاري التمرير...")
            
            if is_first_run:
                # في التشغيل الأول: نأخذ آخر 30 فيديو فقط
                print(f"    (التشغيل الأول - سيتم جلب آخر 30 فيديو)")
                for i in range(5):
                    page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
                    time.sleep(2)
                    print(f"    التمرير {i + 1}/5")
            else:
                # في التشغيلات اللاحقة: نتمرر حتى نجد رابط مكرر
                print(f"    (البحث عن فيديوهات جديدة...)")
                ...
                while not found_duplicate and scroll_count < max_scrolls:
                    ...
                    for video_url in current_videos:
                        if existing_links and video_url in existing_links:
                            ...
                            print(f"    ✓ تم العثور على رابط مكرر - التوقف عن البحث")
                            break
                    
                    if not found_duplicate:
                        print(f"    التمرير {scroll_count}/{max_scrolls}")
            
            page.evaluate("window.scrollTo(0, 0)")
            time.sleep(2)
            
            print(f"[5] جاري استخراج البيانات...")
            ...
            print(f"[6] تم العثور على {len(videos_data)} فيديو")
            ...
            if is_first_run:
                ...
                print(f"[7] معالجة آخر {len(videos_to_process)} فيديو...")
            else:
                ...
                print(f"[7] تم العثور على {new_videos_count} فيديو جديد")
            ...
            for i, video_data in enumerate(videos_to_process, 1):
                ...
                print(f"    [{i}/{len(videos_to_process)}] {video_id} {status}")
    ...
    for username in users:
        ...
        if videos:
            ...
            if is_first_run:
                ...
                # التشغيلات اللاحقة: إضافة الروابط الجديدة في الأعلى مع فاصل
                if new_links:
                    all_users_data[username] = new_links + ['---'] + existing_links
                else:
                    all_users_data[username] = existing_links
            
            all_users_videos[username] = videos
            
            print(f"✓ تمت معالجة @{username}: {len(videos)} فيديو")
        else:
